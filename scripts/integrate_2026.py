...
def integrate_zip(zip_path: Path, year: int = 2026):
    if not zip_path.exists():
        print(f"[ERROR] {zip_path} does not exist!")
        return 0

    extract_dir = zip_path.parent / "extracted_part2"
    extract_dir.mkdir(parents=True, exist_ok=True)
    dest_dir = Path(r"D:\Photos_Merged") / str(year)
    dest_dir.mkdir(parents=True, exist_ok=True)
    report_file = Path(r"D:\_Reports\chrome_bot_download_journal.csv")

    print(f"\n[1/3] Extracting {zip_path.name} ({zip_path.stat().st_size / (1024*1024):.2f} MB)...")
    with zipfile.ZipFile(zip_path, 'r') as z:
        z.extractall(extract_dir)
    print(f"  Extraction complete to: {extract_dir}")

    # Index sidecar JSONs
    print("[2/3] Indexing Google Photos sidecar JSON metadata...")
    json_meta = {}
    ...
    # Process media files
    print(f"[3/3] Standardizing and integrating media into {dest_dir}...")
    ...
    for root, _, files in os.walk(extract_dir):
        for f in files:
            ...
            if integrated_count % 10 == 0 or integrated_count <= 5:
    ...
    print("\n" + "=" * 60)
    print(f"🎉 2026 Part 2 Integration Complete!")
    print(f"  - Integrated: {integrated_count:,} files")
    print(f"  - Skipped (identical duplicates): {skipped_count:,} files")
    print(f"  - Destination: {dest_dir}")
    print("=" * 60)
    return integrated_count

if __name__ == "__main__":
    zip_p = Path(r"D:\_GooglePhotos_Recent_Downloads\2026\GooglePhotos_2026_part2.zip")
    integrate_zip(zip_p, 2026)
